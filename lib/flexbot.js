...
module.exports = (function () {
  ...
  var COMMAND_HEADER = new Buffer([0x24, 0x4d, 0x3c, 0x00]),
      ...
      THROTTLE_CURRENT = new Buffer([0x1d, 0x05, 0xf0]); //If I change the ratios manually, it fails after a couple of seconds. Wonder why.
  ...
  that.connect = function (callback) {
    ...
    that.disarm = function(callback){
      ...
      BLECharacteristic.write(DISARM_COMMAND, true, function(error){
        if(error){
          if(callback) callback(error);
        } else {
          if(callback) callback(undefined);
    ...
    that.trimUp = function(callback){
      ...
      BLECharacteristic.write(TRIM_UP_COMMAND, true, function(error){
        if(error){
          if(callback) callback(error);
        } else {
          if(callback) callback(undefined);
    ...
    that.trimDown = function(callback){
      ...
      BLECharacteristic.write(TRIM_DOWN_COMMAND, true, function(error){
        if(error){
          if(callback) callback(error);
        } else {
          if(callback) callback(undefined);
    ...
    that.trimLeft = function(callback){
      ...
      BLECharacteristic.write(TRIM_LEFT_COMMAND, true, function(error){
        if(error){
          if(callback) callback(error);
        } else {
          if(callback) callback(undefined);
